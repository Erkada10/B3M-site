...
function logout() {
  user = null; curView = 'page'; curPage = 1; editing = false;
...
// === LOCALSTORAGE ===
function getDrafts(sk) { try { return JSON.parse(localStorage.getItem(sk) || '{}'); } catch(e) { return {}; } }
function saveDrafts(sk, d) { localStorage.setItem(sk, JSON.stringify(d)); }

// === СТРАНИЦЫ ===
function getAllPages() {
  var d = getDrafts(SK_PAGES), pages = {};
...
function getPageNums() { return Object.keys(getAllPages()).map(Number).sort(function(a,b){ return a-b; }); }
function isPagePub(n) { return PUBLISHED_PAGES.hasOwnProperty(String(n)); }
function isPageModified(n) {
  var d = getDrafts(SK_PAGES), k = String(n);
...
// === НОВОСТИ ===
function getAllNews() {
  var d = getDrafts(SK_NEWS);
  var news = PUBLISHED_NEWS.slice();
  for (var i = news.length - 1; i >= 0; i--) {
    var id = news[i].id;
    if (d.hasOwnProperty(String(id))) {
      if (d[String(id)] === null) news.splice(i, 1);
      else news[i] = d[String(id)];
    }
  }
  for (var k in d) {
    if (d[k] !== null) {
      var nid = Number(k), found = false;
      for (var j = 0; j < news.length; j++) { if (news[j].id === nid) { found = true; break; } }
      if (!found) news.push(d[k]);
    }
  }
...
// === РЕНДЕР ===
function render() { renderStatus(); renderSidebar(); renderContent(); }

function renderStatus() {
  var s = document.getElementById('status');
  var pc = Object.keys(PUBLISHED_PAGES).length;
  var nc = PUBLISHED_NEWS.length;
  var dp = getDrafts(SK_PAGES), mcp = 0;
  for (var k in dp) { if(dp[k]===null || !PUBLISHED_PAGES.hasOwnProperty(k) || dp[k]!==PUBLISHED_PAGES[k]) mcp++; }
  var dn = getDrafts(SK_NEWS), mcn = 0;
  for (var k2 in dn) { if(dn[k2]!==null) mcn++; }
  s.textContent = 'Админ | Стр: ' + pc + ' (черн: ' + mcp + ') | Новостей: ' + nc + ' (черн: ' + mcn + ')';
}

function renderSidebar() {
  var h = '', nums = getPageNums();

  var news = getAllNews();
  h += '<div class="side-section side-news">';
  h += '<div class="side-header side-header-news">Новости</div>';
  if (news.length === 0) h += '<div class="side-empty">Нет новостей</div>';
  else h += '<div class="side-item side-item-news' + (curView === 'news' ? ' active' : '') + '" onclick="goToNews()">📰 Новости (' + news.length + ')</div>';
  h += '</div>';

  h += '<div class="side-section">';
  h += '<div class="side-header">Страницы</div>';
  if (nums.length === 0) {
    h += '<div class="side-empty">Нет страниц</div>';
  } else {
    h += '<div class="page-select-wrap">';
    h += '<button class="page-select-btn" onclick="togglePageList()">';
    h += '📄 Стр. ' + curPage + ' <span class="page-select-arrow">▼</span>';
    h += '</button>';
    h += '<div class="page-select-list" id="pageSelectList">';
    for (var i = 0; i < nums.length; i++) {
      var n = nums[i], act = (curView === 'page' && n === curPage), badge = '';
      if (isPageModified(n)) badge = ' <span class="badge-dirty">●</span>';
      else if (isPagePub(n)) badge = ' <span class="badge-pub">●</span>';
      h += '<div class="side-item' + (act ? ' active' : '') + '" onclick="goToPage(' + n + ')">Стр. ' + n + badge + '</div>';
    }
    h += '</div>';
    h += '</div>';
  }
...
function renderContent() {
  ...
  if (curView === 'news') { renderNews(el); return; }
  var pages = getAllPages(), nums = getPageNums();
  if (editing && user === 'admin') {
    ...
    el.innerHTML = '<textarea id="earea" style="width:100%;min-height:300px;padding:8px;border:1px solid #c0cad5;border-radius:3px;font-family:inherit;font-size:13px;resize:vertical;outline:none" placeholder="Текст страницы..."></textarea>' +
      '<div style="margin-top:10px;display:flex;gap:8px"><button class="btn btn-s" onclick="saveEdit()">Сохранить черновик</button><button class="btn" onclick="cancelEdit()">Отмена</button></div>';
    var ea = document.getElementById('earea'); ea.value = c; ea.focus();
    return;
  }
  if (nums.length === 0) { el.innerHTML = '<div class="empty-state">Нет страниц</div>'; return; }
  if (!pages.hasOwnProperty(String(curPage))) curPage = nums[0] || 1;
  var pc = pages[String(curPage)] || '', stat = '';
  if (isPageModified(curPage)) stat = '<div style="font-size:11px;color:#e69138;margin-bottom:8px">● Черновик — не виден зрителям</div>';
  else if (isPagePub(curPage)) stat = '<div style="font-size:11px;color:#4bb34b;margin-bottom:8px">● Опубликовано — виден зрителям</div>';
  if (pc.trim() === '') el.innerHTML = stat + '<div class="page-card"><div class="empty-state">Пустая страница ' + curPage + '</div></div>';
  else el.innerHTML = stat + '<div class="page-card">' + esc(pc) + '</div>';
}

function renderNews(el) {
  var news = getAllNews();
  if (news.length === 0) { el.innerHTML = '<div class="empty-state">Нет новостей</div>'; return; }
  var h = '';
  for (var i = 0; i < news.length; i++) {
    var item = news[i];
    h += '<div class="news-card">';
    h += '<h3>' + esc(item.title || 'Без заголовка') + '</h3>';
    h += '<div class="date">' + esc(item.date || '') + '</div>';
    h += '<p>' + esc(item.content || '') + '</p>';
    h += '<div style="margin-top:10px;display:flex;gap:8px">';
    h += '<button class="btn" onclick="editNews(' + item.id + ')">Редактировать</button>';
    h += '<button class="btn btn-d" onclick="deleteNews(' + item.id + ')">Удалить</button>';
    h += '</div>';
    h += '</div>';
  }
  el.innerHTML = h;
...
function newPage() {
  if (editing) { if (!confirm('Выйти из редактирования без сохранения?')) return; editing = false; }
  var nums = getPageNums(), next = nums.length > 0 ? nums[nums.length - 1] + 1 : 1;
  var d = getDrafts(SK_PAGES); d[String(next)] = ''; saveDrafts(SK_PAGES, d);
  curPage = next; curView = 'page'; editing = true; render();
}

function toggleEdit() {
  if (editing || curView !== 'page') return;
  var pages = getAllPages();
  if (!pages.hasOwnProperty(String(curPage))) { var d = getDrafts(SK_PAGES); d[String(curPage)] = ''; saveDrafts(SK_PAGES, d); }
  editing = true; render();
}

function saveEdit() {
  var t = document.getElementById('earea').value;
  var d = getDrafts(SK_PAGES); d[String(curPage)] = t; saveDrafts(SK_PAGES, d);
...
function deletePage() {
  ...
  var pubStatus = isPagePub(curPage) ? 'Эта страница опубликована и видна зрителям.' : 'Эта страница существует только в черновике.';
  showModal('<h2>Удалить страницу ' + curPage + '?</h2>' +
    ...
    '<button class="btn-modal" onclick="confirmDeletePage()">Да, удалить</button><button class="btn-cancel" onclick="closeModal()">Отмена</button>');
}

function confirmDeletePage() {
  var d = getDrafts(SK_PAGES);
  if (isPagePub(curPage)) d[String(curPage)] = null; else delete d[String(curPage)];
  saveDrafts(SK_PAGES, d);
  var nums = getPageNums(), deletedNum = curPage;
  if (nums.length === 0) { curPage = 1; }
  else {
...
function addNews() {
  ...
  setTimeout(function() { document.getElementById('newsTitle').focus(); }, 50);
}
...
function saveNewNews(id, dateStr) {
  ...
  var d = getDrafts(SK_NEWS);
  d[String(id)] = { id: id, title: title, content: content, date: dateStr };
  saveDrafts(SK_NEWS, d);
  closeModal(); curView = 'news'; render();
  toast('Новость создана (черновик)');
}

function editNews(id) {
  var news = getAllNews(), item = null;
  for (var i = 0; i < news.length; i++) { if (news[i].id === id) { item = news[i]; break; } }
  if (!item) return;
  showModal('<h2>Редактировать новость</h2>' +
...
function saveEditNews(id, dateStr) {
  ...
  d[String(id)] = { id: id, title: title, content: content, date: dateStr };
  saveDrafts(SK_NEWS, d);
  closeModal(); render(); toast('Новость сохранена (черновик)');
}
...
function deleteNews(id) {
  ...
  for (var i = 0; i < news.length; i++) { if (news[i].id === id) { item = news[i]; break; } }
  if (!item) return;
  showModal('<h2>Удалить новость?</h2>' +
...
function confirmDeleteNews(id) {
  ...
  d[String(id)] = null;
  saveDrafts(SK_NEWS, d);
  closeModal(); render(); toast('Новость удалена (черновик)');
}

// === ВИДНЫЕ ЗРИТЕЛЯМ ===
function showPublished() {
  var pp = getPageNums().filter(function(n){ return isPagePub(n); });
  var h = '<h2>Видные зрителям</h2>';
  h += '<p style="margin-bottom:10px">Страницы (в файле):</p>';
  if (pp.length === 0) h += '<div style="color:#999;margin-bottom:15px">Нет опубликованных страниц</div>';
  else {
    h += '<div style="margin-bottom:15px">';
    for (var i = 0; i < pp.length; i++) {
      var mod = isPageModified(pp[i]);
      h += '<div style="padding:4px 0">Стр. ' + pp[i] + (mod ? ' <span style="color:#e69138">(есть черновик)</span>' : ' <span style="color:#4bb34b">✓</span>') + '</div>';
    }
    h += '</div>';
  }
  h += '<p style="margin-bottom:10px">Новостей (в файле): ' + PUBLISHED_NEWS.length + '</p>';
  var dn = getDrafts(SK_NEWS), nd = 0;
  for (var k in dn) { if (dn[k] !== null) nd++; }
  if (nd > 0) h += '<div style="color:#e69138;margin-bottom:10px">Черновиков новостей: ' + nd + '</div>';
  h += '<div style="color:#999;font-size:12px;margin-bottom:15px">Нажмите «Скопировать код» и сохраните новый файл, чтобы изменения стали видны.</div>';
  h += '<button class="btn-cancel" onclick="closeModal()">Закрыть</button>';
  showModal(h);
}

// === КОПИРОВАНИЕ КОДА ===
function copyCode() {
  var pages = getAllPages(), cleanPages = {};
  for (var k in pages) { if (pages[k] !== null && pages[k] !== undefined && pages[k] !== '') cleanPages[k] = pages[k]; }
  var news = getAllNews(), cleanNews = [];
  for (var i = 0; i < news.length; i++) {
    if (news[i] && (news[i].title || news[i].content)) {
      cleanNews.push({ id: news[i].id, title: news[i].title, content: news[i].content, date: news[i].date || '' });
    }
  }
  var jsonPages = JSON.stringify(cleanPages);
  var jsonNews = JSON.stringify(cleanNews);
  buildViewerSite(CSS_TEXT, jsonPages, jsonNews);
}
...
function buildViewerSite(cssText, jsonPages, jsonNews) {
  ...
  L.push('</div>');
  // --- JS (шаблонные строки — кавычки не ломаются) ---
  L.push('<script>');
  ...
  L.push(`}`);
  // renderSidebar — одинарные кавычки внутри, двойные для атрибутов HTML
...
function showCodeModal(html) {
  showModal('<h2>Код для зрителей</h2>' +
...
function toast(msg) {
  ...
  t.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:10px 20px;border-radius:4px;font-size:13px;z-index:10001;font-family:Tahoma,Arial,sans-serif';
  document.body.appendChild(t); clearTimeout(tt);
  tt = setTimeout(function(){ t.remove(); }, 3000);
}
...
document.addEventListener('keydown', function(e) {
  ...
  if (curView !== 'page') return;
  var nums = getPageNums(); if (nums.length === 0) return;
